...
HTML_WRAPPER = """<div style="overflow-x: auto; border:1px solid #e6e9ef; border-radius: 0.25rem; padding: 5rem">{}</div>"""
...
def main():
    ...
    activities = ["Home", "Summarize", "Classify","NER Check"]
    ...
    if choice == "Home":
        ...
        if st.button("Summarize"):
            ...
            return None
    elif choice == "NER Check":
        st.subheader("Check Entity Present in Your Text ")
        raw_text = st.text_area("Enter Your Text", "Type Here or Paste")

        if st.button("Check Entities"):
            doc = analyze_ENR(raw_text)
